...
# ann_root = '../shared-nvme/TAD/test.json'

ann_root = '../shared-nvme/shared-nvme/test.json'

# This is the folder for loading the saved vision features
vision_folder = '../shared-nvme/TAD/TAD_test_vision_feature/'
# This is the folder for saving the segment-level scores
score_folder = './TAD_after_InterVL2-8B_test/'
...
# annotation = [obj for obj in annotation if 'Abuse' in obj['video']]

for v_i in range(len(annotation)):
    sample =  annotation[v_i]
    key = sample['video'].split('/')[-1]
    print(key)
